...
def readfromconfig():
    ...
    try:
        with open('/home/pi/Bixels/config.txt', 'r') as config:
            for line in config:
                if len(line) > 3:
                    (key, val) = line.split(',')
                    dicts[key] = val
...
def writetoconfig():
    ...
    if allowconfigwrite == 1:
        try:
            with open('/home/pi/Bixels/config.txt', 'w') as config:
...
def gifupdate():
    os.system('sudo rm /home/pi/Bixels/gifstills/*')
    print('Making GIF...')
    mo=1+math.floor(len(glob.glob("/home/pi/Bixels/stills/*.jpg"))/maxgifimages)
    for it, file in enumerate([os.path.basename(x) for x in glob.glob("/home/pi/Bixels/stills/*.jpg")]):
        if it%mo == 0:
            os.system('sudo cp /home/pi/Bixels/stills/%s /home/pi/Bixels/gifstills/%s' % (file, file))
    os.system('sudo convert -delay 40 -loop 0 /home/pi/Bixels/gifstills/* /home/pi/Bixels/static/timelapse.gif')
...
#    camera.close()
    ...
    camera.capture('/home/pi/Bixels/stills/Snap'+datepic+timenowpic+'.jpg')
    print('Picture taken')
    camera.close()
    print('Camera Closed')
    os.system('sudo cp /home/pi/Bixels/stills/Snap%s%s.jpg /home/pi/Bixels/static/Latest.jpg' % (datepic,timenowpic))

def writetolog(mins, temp, hum):
    with open('/home/pi/Bixels/thlog.txt', 'a+') as log:
...
def plotgraph():
    ...
    Atemp = np.empty((0,2))
    Ahum = np.empty((0,2))
    with open('/home/pi/Bixels/thlog.txt', 'r') as log:
        for line in log:
            if len(line) > 3:
    ...
    plt.savefig('/home/pi/Bixels/static/plotTempHum.png')
    plt.close()
...
#Read config file
if os.path.exists("/home/pi/Bixels/config.txt"):
    try:
        with open('/home/pi/Bixels/config.txt', 'r') as config:
            for line in config:
                if len(line) > 3:
...
if int(dicts['CronCount']) == 0:
    open('/home/pi/Bixels/thlog.txt', 'w+').close()
    open('/home/pi/Bixels/output.txt', 'w+').close()
...
#    open('output.txt', 'w').close()
    os.system('sudo mkdir /home/pi/Bixels/backupstills%s%s' % (datepic, timenowpic))
    os.system('sudo mkdir /home/pi/Bixels/backupfiles%s%s' % (datepic, timenowpic))
    os.system('sudo mv /home/pi/Bixels/stills/* /home/pi/Bixels/backupstills%s%s/' % (datepic, timenowpic))
    try:
        os.system('sudo mv /home/pi/Bixels/static/timelapse.gif /home/pi/Bixels/backupfiles%s%s/timelapse%s%s.gif' % (datepic, timenowpic, datepic, timenowpic))
    except:
        print('No timelapse gif to move')
    os.system('sudo cp /home/pi/Bixels/thlog.txt /home/pi/Bixels/backupfiles%s%s/thlog%s%s.txt' % (datepic, timenowpic, datepic, timenowpic))
    os.system('sudo cp /home/pi/Bixels/output.txt /home/pi/Bixels/backupfiles%s%s/output%s%s.txt' % (datepic, timenowpic, datepic, timenowpic))
#    writetoconfig()
    fanoff()
    pixels.fill((0,0,0))
    pixels.show()
